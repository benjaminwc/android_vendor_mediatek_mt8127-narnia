...
static ssize_t power_supply_show_property(struct device *dev,
	...
	//Barkly, sysfs node for dump charger IC registers
	else if (off == POWER_SUPPLY_PROP_ChargerReg)
		return sprintf(buf, "%s\n", value.strval);
	//Barkly, sysfs node for AC-in wakelock
	else if (off == POWER_SUPPLY_PROP_AC_In_wakelock)
		return sprintf(buf, "%d\n", value.intval);
	//Barkly, sysfs node for enable/disable charging
	else if (off == POWER_SUPPLY_PROP_ChargingOff)
		return sprintf(buf, "%d\n", value.intval);
...
static struct device_attribute power_supply_attrs[] = {
	...
	//Barkly, sysfs node for dump charger IC registers
	POWER_SUPPLY_ATTR(ChargerReg),
	//Barkly, sysfs node for AC-in wakelock
	POWER_SUPPLY_ATTR(AC_IN_wakelock),
	//Barkly, sysfs node for enable/disable charging
	POWER_SUPPLY_ATTR(ChargingOff),
};
					   ...
					   int attrno)
{
	...
	//Barkly, sysfs node for enable/disable charging
	if (attrno == POWER_SUPPLY_PROP_ChargingOff)
	{
		//Set access right = 770 (RW)
		//mode  = S_IRUSR | S_IRGRP | S_IROTH | S_IWUSR | S_IWGRP | S_IWOTH;
		//Barkly, access right = 660 (others not allowed) for Android compatibility test
