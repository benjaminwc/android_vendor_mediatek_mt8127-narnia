...
#           if(search_PartNumber( $MemoryDeviceList[$iter]->{Part_Number} ) )
	...
	foreach $ID (sort by_length (keys(%InFileChip))){
		...
		elsif($MLC_NAND_SUPPORT eq "yes")
		{
			...
			{
			  printf $FD "\n\t\t{$MemoryDeviceList[$it]->{PP_Function}, {$MemoryDeviceList[$it]->{Set_Feature},$MemoryDeviceList[$it]->{Get_Feature},$MemoryDeviceList[$it]->{Pre_Cmd},$MemoryDeviceList[$it]->{Rrtry_Number},$MemoryDeviceList[$it]->{Rrtry_Addr},$MemoryDeviceList[$it]->{Rrtry_Default},$MemoryDeviceList[$it]->{Rrtry_Start},$MemoryDeviceList[$it]->{Read_Type},{$MemoryDeviceList[$it]->{Int_Address}, $MemoryDeviceList[$it]->{Sync}},{$MemoryDeviceList[$it]->{Int_Address}, $MemoryDeviceList[$it]->{Async}}},\n";
