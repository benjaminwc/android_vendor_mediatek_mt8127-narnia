    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mRateSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> arg0, View arg1,
                ...
                mPreambleSpinner.setSelection(0);
                uiUpdateTxPower();
    ...
    private void updateChannels() {
            ...
            if (mLastRateGroup != mRate.getUcRateGroupEep()) {
                if (RateInfo.EEPROM_RATE_GROUP_CCK == mLastRateGroup) {
                    if (mBandwidthIndex == BANDWIDTH_INDEX_40) {
                        mChannel.insertBw40MChannelsIntoAdapter(mChannelAdapter);
        ...
        if (mLastBandwidth != mBandwidthIndex) {
            if (mBandwidthIndex == BANDWIDTH_INDEX_40) {
                ...
                if (mRate.getUcRateGroupEep() != RateInfo.EEPROM_RATE_GROUP_CCK) {
                    mChannel.remove5GChannelsFromAdapter(mChannelAdapter);
                    mChannel.insertBw40MChannelsIntoAdapter(mChannelAdapter);
                }
                bUpdateWifiChannel = true;
            }
            if (mLastBandwidth == BANDWIDTH_INDEX_40) {
                mChannel.insertBw40mUnsupported2dot4GChannels(mChannelAdapter);

                if (mRate.getUcRateGroupEep() != RateInfo.EEPROM_RATE_GROUP_CCK) {
                    mChannel.removeBw40MChannelsFromAdapter(mChannelAdapter);
                    mChannel.insert5GChannelsIntoAdapter(mChannelAdapter);
                }
                bUpdateWifiChannel = true;
            }
        }
