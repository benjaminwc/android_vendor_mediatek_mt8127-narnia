...
public class CapabilityUtils {
	 ...
	 static private Logger logger = Logger.getLogger("CapabilityUtils");
	...
	public static List<String> getSupportedFeatureTags(boolean richcall, boolean ipcall) {
		...
		if (logger.isActivated()) {
    			logger.debug("options getSupportedFeatureTags ");
            }

		if(richcall){
			if (logger.isActivated()) {
    			logger.debug("options getSupportedFeatureTags richcall");
            }
		}
        ...
        if (RcsSettings.getInstance().isVideoSharingSupported() 
            ...
            if (logger.isActivated()) {
    			logger.debug("options getSupportedFeatureTags Video enabled");
            }
		...
		// Image share support
		if (RcsSettings.getInstance().isImageSharingSupported() && isFileStorageAvailable()) {
			if (logger.isActivated()) {
    			logger.debug("options getSupportedFeatureTags Image enabled");
            }
    ...
    public static Capabilities extractCapabilities(SipMessage msg) {
    	...
    	for(int i=0; i < tags.size(); i++) {
        	...
        	if (tag.contains(FeatureTags.FEATURE_RCSE_IMAGE_SHARE)) {
				if (logger.isActivated()) {
    			logger.debug("options extractCapabilities Image enabled");
            }
    		...
    		if (tag.startsWith(FeatureTags.FEATURE_RCSE + "=\"" + FeatureTags.FEATURE_RCSE_EXTENSION)) {
				...
				if (logger.isActivated()) {
					logger.debug("options extractCapabilities Joyn Offline");
				}
	        ...
	        if (video | image) {
		        ...
		        if (video) {
					...
					for(int i=0; i < videoFormats.size(); i++) {
						...
						videoSharingConfig.append("a=fmtp:" + fmt.getPayload()+ " "+ "profile-level-id=42900b;packetization-mode=1"+ SipUtils.CRLF);
					}
