...
class CheckPkg {
    ...
    private boolean checkVersion(ZipFile zipFile, int otaType) {
        ...
        if (((infoPkg.mDevNum == null) && (infoPkg.mProNum == null)) || (otaType == DELTA_PKG)
            ...
            Xlog.i(TAG, "infoPkg.mDevNum " + infoPkg.mDevNum + " infoPkg.mProNum "
                    + infoPkg.mProNum + " otaType " + otaType + " infoPkg.mFingerPrint1 "
                    + infoPkg.mFingerPrint1 + " infoPkg.mFingerPrint2 "
                    + infoPkg.mFingerPrint2);
            Xlog.e(TAG, "project number or finger print of upgrade package is null,"
                    + "no need to compare.");
            return true;
        }
    ...
    private boolean comparePartition(List<PartitionInfo> partitionDev, List<PartitionInfo> partitionPkg, int otaType) {
       ...
       // and device is encrypted.
       boolean notifyStatus = DownloadInfo.getInstance(mContext).getNotifyStatus();
        if (!notifyStatus && Util.isEncrypted() && otaType != DELTA_PKG) {   
            for (int i = 0; i < length; i++) {
                if (partitionDev.get(i).mName.equalsIgnoreCase("userdata")
                        || partitionDev.get(i).mName.equalsIgnoreCase("usrdata")) {
                    if (partitionDev.get(i).mOffset != partitionPkg.get(i).mOffset ) {      
        ...
        try {
            ...
            while ((line = bfReader.readLine()) != null) {
                ...
                if (FeatureOption.MTK_EMMC_SUPPORT) {
                    ...
                    info = new PartitionInfo(partitionInfoArray[0],
                            isComboEmmcPreloader ? extractSize(partitionInfoArray[1]) : offset);
                    Xlog.i(TAG, "EMMC partition layout:name = " + info.mName + " offset = "
                            + info.mOffset);
                    ...
                    if (FAT_PARTITION_NAME.equalsIgnoreCase(partitionInfoArray[0])) {
                        break;
                    }

                    offset = isComboEmmcPreloader ? 0
                            : (offset + extractSize(partitionInfoArray[1]));
                } else { // for NAND:compare between start address of
                         // dumchar_info and value in scatter.txt
                    info = new PartitionInfo(partitionInfoArray[0],
                            extractSize(partitionInfoArray[2]));
                    Xlog.i(TAG, "NAND partition layout: name = " + info.mName + " offset = "
                            + info.mOffset);
                    partitionDev.add(info);
                }
