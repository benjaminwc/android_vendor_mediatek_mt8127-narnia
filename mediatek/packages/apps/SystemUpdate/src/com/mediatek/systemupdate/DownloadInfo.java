...
final class DownloadInfo {
    ...
    /**
     * Get a flag to indicate if has notify user back up data.
     *
     * @return true if has notified, else false.
     */
    boolean getNotifyStatus() {
        boolean hasNotify = mPreference.getBoolean(HAS_NOTIFY, false);
        Xlog.i(TAG, "getNotifyStatus, get result = " + hasNotify);
        return hasNotify;
    }

    /**
     * Set a flag to indicate if has notify user to backup.
     *
     * @param status
     *            the status to set. True if has notified.
     */
    void setNotifyStatus(boolean hasNotify) {
        Xlog.i(TAG, "setNotifyStatus, hasNotify = " + hasNotify);
